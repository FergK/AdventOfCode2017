...
// var input = `azz ajdcqkd bcafn zaz dcjaqdk gylyzo zaz
// xzvfbf fopmfxu mvftgr mfupoxf coyhof talcc vpkslo`;
...
function checkPhrase(phrase) {
  ...
  for (var i = 0; i < phrase.length; i++) {
    for (var j = i+1; j < phrase.length; j++) {
      if ((phrase[i] == phrase[j]) || checkAnagram(phrase[i], phrase[j])) {
        console.log("Invalid: " + phrase[j]);
        return false;
      }
    }
  }
...
input = input.split("\n");
var passphrases = input.map( function(r){ return r.split(" ") } );
console.log(passphrases);

var totalValid = 0;
for (var i = 0; i < passphrases.length; i++) {
  if (checkPhrase(passphrases[i])) {
    totalValid++;
  }
}

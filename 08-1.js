...
function doInstruction(registers, ins) {
	let destination = ins[0];
	let modifier = ins[1];
	let modValue = parseInt(ins[2], 10);
	let source = ins[4];
	let conditional = ins[5];
	let condValue = parseInt(ins[6], 10);

	if ( registers[destination] === undefined ) {
		registers[destination] = 0;
	} 

	if ( registers[source] === undefined ) {
		registers[source] = 0;
	} 
	...
	switch (conditional) {
		case '==':
			doIf = (registers[source] == condValue);
			break;
		case '!=':
			doIf = (registers[source] != condValue);
			break;
	...
	if (doIf) {
		switch (modifier) {
			case 'inc':
				registers[destination] += modValue;
				break;
			case 'dec':
				registers[destination] -= modValue;
				break;
...
for (var i = 0; i < input.length; i++) {
	doInstruction(registers, input[i]);
...
console.log("Max value in registers: " + max);	
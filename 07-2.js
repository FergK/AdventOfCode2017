...
// Parse each row in the text file into an object and place all of those into an object containing all the discs
function parseRow(r) {
	r = r.split(" ");
	let disc = {};
	disc.name = r[0];
	disc.weight = parseInt(r[1].substring(1, r[1].length - 1), 10);
	disc.parent = null;
	disc.children = null;
...
for (let i in input) {
	let newDisc = parseRow(input[i]);
	discs[newDisc.name] = newDisc;
}

// Loop through all the discs and connect each one to it's parent
for (let i in discs) {
	if (discs[i].children !== null) {
		for (let j in discs[i].children) {
			discs[j].parent = discs[i].name;
			discs[i].children[j] = discs[j];
		}
	}
}

// The bottom disc is the one without a parent. Find it!
let bottom = null;
for (let i in discs) {
	if (discs[i].parent == null) {
		bottom = discs[i];
		break;
	}
}
// console.log("Bottom disc is " + bottom.name);

// Recursively find the total weights of each tower and if it is unbalanced
function findTotalWeight(disc) {
	...
	let childWeights = [];
	for (let i in disc.children) {
		let childWeight = findTotalWeight(disc.children[i]);
		childWeights.push(childWeight);
		disc.totalWeight += childWeight;
	}

	// This is some fancy code to detect if the child weights have more than one unique value
	if ((childWeights.filter((v, i, a) => a.indexOf(v) === i)).length > 1) {
		// console.log( disc.name + " child weights are: " + childWeights );
...
findTotalWeight(bottom);

// Recursively find the highest unbalanced child
function findUnbalancedChild(disc) {
	for (let i in disc.children) {
		if ( disc.children[i].balanced === false ) {
			// console.log( disc.name + " is parent of unbalanced child " + disc.children[i].name);
			return findUnbalancedChild(disc.children[i]);
		}
	}
	return disc;
}

let unbalanced = findUnbalancedChild(bottom);

// Find which of the children are the odd one out
childWeights = {};
for (let i in unbalanced.children) {
	if (childWeights[unbalanced.children[i].totalWeight] === undefined ) {
		childWeights[unbalanced.children[i].totalWeight] = [];
	}
	childWeights[unbalanced.children[i].totalWeight].push(unbalanced.children[i].name);
}

let oddOne, goodOne;
for (let i in childWeights) {
	if (childWeights[i].length === 1) {
		oddOne = childWeights[i][0];
	} else {
		goodOne = childWeights[i][0];
	}
}
// console.log(oddOne);
// console.log(goodOne);
console.log(discs[oddOne].weight - ( discs[oddOne].totalWeight - discs[goodOne].totalWeight )  )
